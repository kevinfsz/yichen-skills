...
3. 不得把“读取”推断成“下载”，也不得把“下载”推断成登录态授权。仅执行用户当轮明确要求的动作。
...
- 是否需要登录态；如需要，必须按目标平台 Skill 的规则取得当轮授权。
...
```json
{"item_ref":"opaque-or-line-ref","platform":"xiaohongshu","source_url_ref":"input.txt:3","requested_actions":["archive"],"status":"success","artifact_paths":["/absolute/path"],"route_skill":"yichen-xiaohongshu-fetch","fetched_at":"RFC3339"}
```
